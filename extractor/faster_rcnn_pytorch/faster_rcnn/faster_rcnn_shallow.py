...
class RPN(nn.Module):
    ...
    def __init__(self):
        ...
        self.features = ShallowNet()
        self.conv1 = Conv2d(128, 128, 3, same_padding=True)
        self.score_conv = Conv2d(128, len(self.anchor_scales) * 3 * 2, 1, relu=False, same_padding=False)
        self.bbox_conv = Conv2d(128, len(self.anchor_scales) * 3 * 4, 1, relu=False, same_padding=False)
...
class ShallowRCNN(nn.Module):
    ...
    def __init__(self, classes=None, debug=False):
        super(ShallowRCNN, self).__init__()
        ...
        self.rpn = RPN()
        self.roi_pool = RoIPool(7, 7, 1.0/2)
        self.fc6 = FC(128 * 7 * 7, 4096)
