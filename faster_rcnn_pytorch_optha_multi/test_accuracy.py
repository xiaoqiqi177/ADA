...
if __name__ == '__main__':
    ...
    for img_id, (cls_dets_all, gt_dets) in enumerate(zip(detected_bboxes, gt_bboxes)):
        ...
        for bbox_detected in cls_dets_all:
            for gt_id, bbox_gt in enumerate(gt_dets):
                ...
                if img_id == 35 and gt_id == 2:
                    print(gt_id, ':', bbox_detected, bbox_gt[-1], iou_value)
                if iou_value > 0.5:
                    overlap[gt_id] = 1
        ...
        if detected_bb > 0:
            y_pred.append(np.clip(cls_dets_all[:,-1].sum()/20., 0., 1.))
        else:
            y_pred.append(0.)
    ...
    print('AUC: ', results[0])
    print('AP: ', results[1])
    print('avg_recall: ', recalls.mean())
    print('total_recall: ', sum_overlaps / sum_gt_bbs)
    print('avg_precision: ', precisions.mean())
    print('total_precison: ', sum_overlaps / sum_detected_bbs)
