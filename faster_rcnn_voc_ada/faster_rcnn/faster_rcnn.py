...
class RPN(nn.Module):
    ...
    def forward(self, im_data, im_info, gt_boxes=None, gt_ishard=None, dontcare_areas=None):
        im_data = network.np_to_variable(im_data, is_cuda=global_is_cuda)
    ...
    def proposal_layer(rpn_cls_prob_reshape, rpn_bbox_pred, im_info, cfg_key, _feat_stride, anchor_scales):
        ...
        x = network.np_to_variable(x, is_cuda=global_is_cuda)
        return x.view(-1, 5)
    ...
    def anchor_target_layer(rpn_cls_score, gt_boxes, gt_ishard, dontcare_areas, im_info, _feat_stride, anchor_scales):
        ...
        rpn_labels = network.np_to_variable(rpn_labels, is_cuda=global_is_cuda, dtype=torch.LongTensor)
        rpn_bbox_targets = network.np_to_variable(rpn_bbox_targets, is_cuda=global_is_cuda)
        rpn_bbox_inside_weights = network.np_to_variable(rpn_bbox_inside_weights, is_cuda=global_is_cuda)
        rpn_bbox_outside_weights = network.np_to_variable(rpn_bbox_outside_weights, is_cuda=global_is_cuda)
...
class FasterRCNN(nn.Module):
    ...
    def proposal_target_layer(rpn_rois, gt_boxes, gt_ishard, dontcare_areas, num_classes):
        ...
        rois = network.np_to_variable(rois, is_cuda=global_is_cuda)
        labels = network.np_to_variable(labels, is_cuda=global_is_cuda, dtype=torch.LongTensor)
        bbox_targets = network.np_to_variable(bbox_targets, is_cuda=global_is_cuda)
        bbox_inside_weights = network.np_to_variable(bbox_inside_weights, is_cuda=global_is_cuda)
        bbox_outside_weights = network.np_to_variable(bbox_outside_weights, is_cuda=global_is_cuda)
    ...
    def extract(self, image, dets):
        im_data, im_scales = self.get_image_blob(image)
        im_info = np.array(
            [[im_data.shape[1], im_data.shape[2], im_scales[0]]],
            dtype=np.float32)
        features = self.extractfeatures(im_data, im_info, dets)
        return features
