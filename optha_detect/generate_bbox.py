            ...
            for i in range(h_num):
                for j in range(w_num):
                    ...
                    for c_id, contour in enumerate(contours):
                        ...
                        x, y = x+1, y+1
                        if DEBUG:
                            cv2.drawContours(new_annotation, [contour], 0, (255, 0, 0), 2)
                            cv2.rectangle(new_annotation, (x, y), (x + w, y + h), (0, 255, 0), 2)
                            cv2.rectangle(new_img, (x, y), (x + w, y + h), (0, 255, 0), 2)
                            cv2.putText(new_img, str(c_id), (x, y + 15), cv2.FONT_HERSHEY_PLAIN, 2., (0, 0, 255), thickness=1)
                        bboxes.append([x, y, x+w, y+h])
                    if DEBUG:
